...
@objc public protocol RAReorderableLayoutDelegate: UICollectionViewDelegateFlowLayout {
        ...
        optional func collectionView(collectionView: UICollectionView, collectionViewLayout layout: RAReorderableLayout, willDeleteItemAtIndexPath indexPath: NSIndexPath)
}
...
public class RAReorderableLayout: UICollectionViewFlowLayout, UIGestureRecognizerDelegate {
        ...
        public func cancelDrag() {
                self.cancelDrag(toIndexPath: nil, withCanceledPoint: nil)
        }

        private func cancelDrag(toIndexPath toIndexPath: NSIndexPath!, withCanceledPoint canceledPoint: CGPoint?) {
                if self.cellFakeView == nil {
                        return
                }
                ...
                if let dropFrame = self.dropFrame, canceledPoint = canceledPoint {
                        if dropFrame.contains(canceledPoint) {
                                // drop cell
                                self.delegate?.collectionView?(self.collectionView!, collectionViewLayout: self, willDeleteItemAtIndexPath: toIndexPath)
                                self.collectionView!.deleteItemsAtIndexPaths([toIndexPath])
                                self.endDrag(toIndexPath)
                                return
                        }
                }
        ...
        private func endDrag(toIndexPath: NSIndexPath!) {
        ...
        internal func handleLongPress(longPress: UILongPressGestureRecognizer!) {
                ...
                case .Cancelled:
                        fallthrough
                case .Ended:
                        self.cancelDrag(toIndexPath: indexPath, withCanceledPoint: longPress.locationInView(self.collectionView!.superview))
                default:
                        break
                }
        }
